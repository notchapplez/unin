...
pub fn compile_gleam(path: PathBuf, noinstall: bool) {
    ...
    for line in bufreader.lines() {
            ...
            || line.contains("Exported")
        {
			...
			let mut colored_vec: Vec<String> = sauce.iter()
	...
	let path_to_shipment_dest = PathBuf::from(format!("/usr/local/bin/.unin_gleam/{}", package_name));

	let copier = cmd!("sudo", "cp", "-r", &path_to_shipment, &path_to_shipment_dest)
		.stderr_to_stdout()
		.dir(path.clone())
		.run();
	...
	let script = format!("#!/usr/bin/bash\n/usr/local/bin/.unin_gleam/{}/entrypoint.sh run", package_name);
